...
/*
	...
	{
		//lightPos.y += 0.1f;
		static float coneAngle = 10.0f;
		static float coneDirectionArray[3] = { -10.0f, 0.0f, 0.0f };
		glm::vec3 coneDirection = glm::vec3(coneDirectionArray[0], coneDirectionArray[1], coneDirectionArray[2]);
...

		{
			...
			ImGui::Begin("Param");
			ImGui::SliderFloat("Cone Angle", &coneAngle, 1.0f, 44.0f, "%.3f");            // Edit 1 float using a slider from 0.0f to 1.0f
		...
		/*{
			glBindFramebuffer(GL_FRAMEBUFFER, 0);
			glViewport(0, 0, SCREEN_WIDTH * 3 / 4, SCREEN_HEIGHT * 3 / 4);
			...
			glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (GLfloat)SHADOW_WIDTH / (GLfloat)SHADOW_HEIGHT, 0.1f, 1000.0f);
			...
			glBindTexture(GL_TEXTURE_2D, depthMap);
			cyborg.Draw(quadShader);
		}*/

		// 从光照10,0,0的位置渲染模型。存深度
		glm::mat4 lightSpaceMatrix;
		{
			//static glm::vec3 aaaa = glm::vec3(0.0f);
			//aaaa.y += 0.1f;
			//lightPos.y += 0.1f;
			//printf("aaa=%f\n", aaaa.x);
			...
			lightView = glm::lookAt(lightPos, coneDirection, glm::vec3(0.0, 1.0, 0.0));
			lightSpaceMatrix = lightProjection * lightView;
			glUniformMatrix4fv(glGetUniformLocation(simpleDepthShader.Program, "lightSpaceMatrix"), 1, GL_FALSE, glm::value_ptr(lightSpaceMatrix));
			
			GLint modelLoc = glGetUniformLocation(simpleDepthShader.Program, "model");
			glm::mat4 model;// = glm::scale(glm::mat4(1.0f), glm::vec3(0.3));
