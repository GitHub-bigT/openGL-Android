...
#define NUM_SAMPLES_RCP 0.002;//0.001953125
...
//		return evaluateLight(pos);//max(0.0,dot(normal,Lnorm)) * evaluateLight(pos);
//	return vec3(0,0,0);
//}
...
void main()
{
	...
	for(float l = marchingRayLength; l > stepSize; l -= stepSize)
	{
		...
		float shadowTerm = 1;//ShadowCalculation(curPosLightSpace, curPos) ;// 阴影计算,no

		float d = length(curPos- lightPos);
		float dRcp = 1.0/d;// 倒数，reciprocal

		// 锥体
		vec3 rayDirection = normalize(curPos-lightPos);//(curPos-lightPos)*dRcp;
	...
	vec3 cc = VLI * lightColor;
	FragColor = vec4(cc, 1.0f);
	//FragColor = vec4(VLI, 0, 0, 1.0f);//vec4(pow(gammacolor, vec3(1.0/2.2)) ,1.0); 
